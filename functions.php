...
function r34ics_date_format($format='', $short_day=false) {
...
function r34ics_feed_colors_css($ics_data, $padding=false, $hover=false) {
	if (!empty($ics_data['colors']) || !empty($ics_data['tablebg'])) {
		ob_start();
...
function r34ics_lightbox_container($echo=true) {
	$output = '<div class="r34ics_lightbox"><div class="r34ics_lightbox_close">&times;</div><div class="r34ics_lightbox_content"></div></div>';
...
function r34ics_location_map_link($location='') {
	...
	// Treat the entire string as a Google Maps search
...
function r34ics_purge_calendar_transients() {
	...
	global $wpdb;
	$sql =	"DELETE FROM `" . $wpdb->options . "` WHERE `option_name` LIKE '%\_transient\_%' AND `option_name` LIKE '%_R34ICS::%'";
...
/*
Note: This is not a true GUID/UUID; but we don't need anything cryptographically secure, just a reasonably reliably unique string on the page. This produces a much shorter string than the old method (and possibly generates faster), which is useful in cases where we insert a LOT of these in the HTML output.
*/
...
function r34ics_url_uniqid($url='') {
	...
	// If the option exists, we return the key, if it's present
	if ($r34ics_feed_urls = get_option('r34ics_feed_urls')) {
		return array_search($url, $r34ics_feed_urls);
	}
...
function _r34ics_debug($arr) {
	...
	foreach ((array)$settings_fields as $field) {
		$arr['WP settings'][$field] = get_option($field);
	}
	ob_start();
	echo '<hr /><pre>';
	print_r($arr);
