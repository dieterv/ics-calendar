...
<section class="<?php echo esc_attr($ics_calendar_classes); ?>" id="<?php echo esc_attr($ics_data['guid']); ?>">
	...
	if (!empty($args['monthnav']) && in_array($args['monthnav'], array('arrows','both','compact'))) {
		?>
		<nav class="ics-calendar-arrow-nav r34ics-hidden-on-load">
	...
	foreach (array_keys((array)$ics_data['events']) as $year) {
		for ($m = 1; $m <= 12; $m++) {
			...
			?>
			<article class="ics-calendar-month-wrapper r34ics-hidden-on-load<?php if ($ym < $this_ym) { echo ' past'; } ?>" data-year-month="<?php echo esc_attr($ym); ?>">
